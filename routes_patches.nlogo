...
to init-car
  ...
  face one-of neighbors4 with [pcolor = yellow]
end

to go
  ask turtles [advance2]
  tick
end

to advance
  let f (patch-set (patch-ahead 1)) with [pcolor = yellow]
  let r (patch-set (patch-at-heading-and-distance (heading + 90) 1)) with [pcolor = yellow]
  let l (patch-set (patch-at-heading-and-distance (heading - 90) 1)) with [pcolor = yellow]
